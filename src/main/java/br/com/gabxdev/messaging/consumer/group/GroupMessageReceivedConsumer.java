...
import static br.com.gabxdev.config.RabbitMQConfig.QueueNames.GROUP_MESSAGE_RECEIVED;

@Component
@RequiredArgsConstructor
public class GroupMessageReceivedConsumer {
    ...
    @RabbitListener(queues = GROUP_MESSAGE_RECEIVED)
    public void processReceivedMessage(MessageWrapper<GroupMessageStatusNotificationRequest> messageWrapper) {
        ...
        groupMessageStatusService.markMessageAsReceived(request.messageId()).ifPresent(groupMessageStatus -> {
            var response = groupMessageMapper.toGroupMessageStatusResponse(groupMessageStatus);
